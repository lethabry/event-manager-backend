...

{
    ...
    private readonly Mock<ICacher> _mockCacher;
...

    {
        ...
        _cacheOptions = new EventCacheOptions

        {

            EventByIdTtl = TimeSpan.FromMinutes(2),

            TopEventsTtl = TimeSpan.FromMinutes(10),
        ...
        _mockCacher.Setup(cacher => cacher.GetDataByKeyAsync<CachedEventDTO>(It.IsAny<string>()))

            .ReturnsAsync((CachedEventDTO?)null);

        _mockCacher.Setup(cacher => cacher.GetDataByKeyAsync<List<CachedEventDTO>>(It.IsAny<string>()))

            .ReturnsAsync((List<CachedEventDTO>?)null);
        ...
        services.AddScoped<IEventRepository>(_ => _mockEventRepository.Object);

        services.AddScoped<IEventValidatorService>(_ => _mockValidationService.Object);

        services.AddScoped<ICacher>(_ => _mockCacher.Object);

        services.AddSingleton<ILogger<EventService>>(Microsoft.Extensions.Logging.Abstractions.NullLogger<EventService>.Instance);

        services.AddSingleton<IOptions<EventCacheOptions>>(Options.Create(_cacheOptions));
    ...
    private void ConfigureRepositoryMock()

    {

        _mockEventRepository.Setup(repository => repository.GetEventsAsync(

                It.IsAny<string?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>(), It.IsAny<int>(), It.IsAny<int>()))

            .ReturnsAsync((string? title, DateTime? from, DateTime? to, int page, int pageSize) =>

                FilterEvents(title, from, to)
        ...
        _mockEventRepository.Setup(repository => repository.GetEventsCountAsync(

                It.IsAny<string?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>()))

            .ReturnsAsync((string? title, DateTime? from, DateTime? to) => FilterEvents(title, from, to).Count());

        _mockEventRepository.Setup(repository => repository.GetEventByIdAsync(It.IsAny<Guid>()))

            .ReturnsAsync((Guid id) => _events.FirstOrDefault(evt => evt.Id == id));

        _mockEventRepository.Setup(repository => repository.CreateEventAsync(It.IsAny<CreateEventDTO>()))

            .ReturnsAsync((CreateEventDTO dto) =>

            {

                var created = EventEntity.Create(dto.Title, dto.StartAt, dto.EndAt, dto.TotalSeats,
        ...
        _mockEventRepository.Setup(repository => repository.UpdateEventAsync(It.IsAny<EventEntity>()))

            .ReturnsAsync((EventEntity updated) => _events.Any(evt => evt.Id == updated.Id) ? updated : null);

        _mockEventRepository.Setup(repository => repository.DeleteEventAsync(It.IsAny<Guid>()))

            .ReturnsAsync((Guid id) =>

            {

                var existing = _events.FirstOrDefault(evt => evt.Id == id);

                return existing != null && _events.Remove(existing);

            });

        _mockEventRepository.Setup(repository => repository.GetTopEventsAsync())

            .ReturnsAsync(() => _events.Take(10).ToList().AsReadOnly());

    }


    private IEnumerable<EventEntity> FilterEvents(string? title, DateTime? from, DateTime? to)

    {

        IEnumerable<EventEntity> query = _events;

        if (!string.IsNullOrEmpty(title))

        {

            query = query.Where(evt => evt.Title.Contains(title, StringComparison.OrdinalIgnoreCase));
    ...
    private static bool MatchesEvent(CachedEventDTO cached, EventEntity evt)

    {

        return cached.Id == evt.Id &&

               cached.Title == evt.Title &&

               cached.Description == evt.Description &&

               cached.StartAt == evt.StartAt &&

               cached.EndAt == evt.EndAt &&

               cached.TotalSeats == evt.TotalSeats &&

               cached.AvailableSeats == evt.AvailableSeats;
...

    {
        ...
        var paginatedEvents = _events.Take(DefaultPageSize).ToList();
    ...
    public async Task GetEventById_CacheMiss_ShouldReadRepositoryAndWriteCache()

    {
        ...
        var cacheKey = GetEventCacheKey(evt.Id);

        _mockCacher.Setup(cacher => cacher.TryWriteDataAsync(

                cacheKey, It.IsAny<CachedEventDTO>(), _cacheOptions.EventByIdTtl))
        ...
        _mockCacher.Verify(cacher => cacher.GetDataByKeyAsync<CachedEventDTO>(cacheKey), Times.Once);

        _mockEventRepository.Verify(repository => repository.GetEventByIdAsync(evt.Id), Times.Once);

        _mockCacher.Verify(cacher => cacher.TryWriteDataAsync(

            cacheKey,

            It.Is<CachedEventDTO>(cached => MatchesEvent(cached, evt)),

            _cacheOptions.EventByIdTtl), Times.Once);
    ...
    public async Task GetEventById_IdExistInCache_ShouldReturnEvent()

    {
        ...
        _mockCacher.Setup(cacher => cacher.GetDataByKeyAsync<CachedEventDTO>(cacheKey))

            .ReturnsAsync(CachedEventDTO.FromEvent(evt));
        ...
        _mockEventRepository.Verify(repository => repository.GetEventByIdAsync(evt.Id), Times.Never);

        _mockCacher.Verify(cacher => cacher.TryWriteDataAsync(

            It.IsAny<string>(), It.IsAny<CachedEventDTO>(), It.IsAny<TimeSpan>()), Times.Never);
...

    {
        ...
        _mockEventRepository.Verify(repository => repository.CreateEventAsync(evnt), Times.Once);

        _mockCacher.Verify(cacher => cacher.TryDeleteDataAsync(_cacheOptions.TopEventsKey), Times.Once);
...

    {
        ...
        _mockCacher.Verify(cacher => cacher.TryWriteDataAsync(
            ...
            GetEventCacheKey(evnt.Id),

            It.Is<CachedEventDTO>(cached => MatchesEvent(cached, result!)),
...

    {
        ...
        var expected = _events.Take(2).ToList();

        _mockCacher.Setup(cacher => cacher.GetDataByKeyAsync<List<CachedEventDTO>>(_cacheOptions.TopEventsKey))

            .ReturnsAsync(expected.Select(CachedEventDTO.FromEvent).ToList());
        ...
        _mockEventRepository.Verify(repository => repository.GetTopEventsAsync(), Times.Never);
    ...
    public async Task GetTopEvents_CacheMiss_ShouldReadRepositoryAndWriteCache()

    {

        //Arrange

        var expected = _events.Take(2).ToList().AsReadOnly();

        _mockEventRepository.Setup(repository => repository.GetTopEventsAsync()).ReturnsAsync(expected);

        _mockCacher.Setup(cacher => cacher.TryWriteDataAsync(

                _cacheOptions.TopEventsKey, It.IsAny<List<CachedEventDTO>>(), _cacheOptions.TopEventsTtl))
        ...
        _mockEventRepository.Verify(repository => repository.GetTopEventsAsync(), Times.Once);

        _mockCacher.Verify(cacher => cacher.TryWriteDataAsync(

            _cacheOptions.TopEventsKey,

            It.Is<List<CachedEventDTO>>(cached => cached.Count == expected.Count &&

                                                       cached.Zip(expected).All(pair => MatchesEvent(pair.First, pair.Second))),

            _cacheOptions.TopEventsTtl), Times.Once);
...

    {
        ...
        _mockEventRepository.Verify(repository => repository.DeleteEventAsync(evnt.Id), Times.Once);

        _mockCacher.Verify(cacher => cacher.TryDeleteDataAsync(GetEventCacheKey(evnt.Id)), Times.Once);
