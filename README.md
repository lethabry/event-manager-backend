...
- Схема базы данных создаётся автоматически при запуске приложения. В `Program.cs` при старте вызывается `db.Database.EnsureCreated()`, который создаёт базу и таблицы по текущей модели EF Core, если их ещё нет.
- Отдельные миграции не используются: достаточно указать корректную строку подключения и запустить приложение — недостающая схема будет создана. Существующая схема при этом не изменяется, поэтому при правках модели пересоздайте базу вручную.
...
- При старте приложение автоматически создаст схему БД (см. раздел «Автоматическое создание схемы БД»).
...
Для работы с базой данных тесты используют InMemory-провайдер EF Core (`Microsoft.EntityFrameworkCore.InMemory`): реальный PostgreSQL для запуска тестов не требуется — каждый тест работает с изолированной базой в памяти.
