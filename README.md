...
4. Дождитесь обработки фонового сервиса: он делает `Task.Delay(2000)` перед подтверждением каждой заявки и повторяет проверку каждые 15 секунд.
5. `GET /bookings/{bookingId}` — после обработки статус станет `Confirmed`.

> На этом спринте фоновый сервис переводит бронирования только в `Confirmed`, отказ (`Rejected`) в реальном сценарии пока не применяется.
